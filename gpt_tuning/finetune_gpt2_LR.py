...
def train_or_pred():
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=0, num_training_steps=len(train_dataset)
    )

    def lambdaLR(current_step):
        return max(
            0.0, 1.0 - (current_step / len(train_dataset)) * training_args.num_train_epochs
        )
